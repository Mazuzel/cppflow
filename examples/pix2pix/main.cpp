...
int main() {
    ...
    //img_data.assign(scaled.data, scaled.data + scaled.total() * scaled.channels());
    ...
    for (int j = 0; j < imgVector.size(); j++){
      float val = imgVector[j];
      pixels[j] = (unsigned char)((val+1.0)*127.5);
    }
